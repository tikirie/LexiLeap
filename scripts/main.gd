...
const debuff = preload("res://scenes/debuff.tscn")
...
func _ready() -> void:
	...
	for i in len(synonyms):
		...
		for j in len(temp):
			...
			#new_label1 for synonyms
			new_label1(temp[j], heights[i], heights[i + 1])
	...
	for i in len(antonyms):
		...
		for j in len(temp):
			...
			#new_label2 for antonyms
			new_label2(temp[j], heights[i], heights[i + 1])
	#for j in len(syn):
		#print(syn[j])
